    ...
    return (
        <div className="grid lg:grid-cols-4 md:grid-cols-2 sm:grid-cols-1 gap-5 justify-end w-full items-center">
            {props.imageUrls.map((imageUrl) => (
                ...
                <Link href="/products" onClick={() => dispatch(urlChange(imageUrl.name)) } class="max-w-sm border rounded-lg shadow-md bg-gray-800 border-gray-700">
                    
                    <a >
                        <img class="rounded-t-lg scale-100 w-fit hover:scale-105 ease-in duration-500" src={imageUrl.productUrl} valt="" />
                    </a>
                    <div class="p-5">
                        <a>
                            <h5 class="mb-3 text-2xl font-bold tracking-tight text-white">{imageUrl.name}</h5>
                        </a>
